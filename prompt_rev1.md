...
{
  "metadata": {
    "generation_error": "<input contradiction detected, or \"\" if none>"
  },
  "sections": {
    "overview": {
      "text": "<summary>",
...
- **Grounding.** Ground every inference in the INPUT. If a field cannot be concluded **confidently** from the INPUT, keep it empty (`""` for strings, `[]` for arrays) — **do not force generation** to avoid hallucination. Never emit `null`. If the CVE is marked rejected, leave all sections empty and record it in `generation_error`.

- **Contradictions.** If fields in the INPUT carry factually conflicting information about the same concrete fact (e.g. different product scope, conflicting version ranges, contradictory patch targets), do not silently resolve the conflict. Record it in `metadata.generation_error`, naming the conflicting fields. Scope the affected guide sections conservatively — prefer the narrower or more restrictive claim. Differences in phrasing, emphasis, or level of detail between sources are not contradictions.
